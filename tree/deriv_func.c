...
struct s_tree *deriv_sqrt(struct s_tree *node)
{
  ...
  new->right = build_function("pow");
  ((struct s_function*)new->right->data)->param = -1;
  new->right->left = build_function("sqrt");
...
{
  if(((struct s_function*)node->data)->param == 2)
    return deriv(node->left);
  struct s_tree *new = build_function("pow");
  ((struct s_function*) new->data)->mult
    = ((struct s_function*) node->data)->mult * ((struct s_function*) node->data)->power;
